...
fn generate_expression(expression: Expression) -> Vec<Instruction> {
    ...
    match expression {
        Expression::Identifier(identifier) => instructions.push(Instruction::LoadIdentifier(identifier)),
        Expression::Integer(integer) => instructions.push(Instruction::LoadInteger(integer)),
    ...
    for statement in statements {
        ...
        match statement {
            Statement::Variable(identifier, expression) => {
                ...
                instructions.push(Instruction::DefineVariable(identifier));
